...
ClassResults1$ClassProportion<-(ClassResults1$Amount/1000)

#Calculate state class average percent for unique StratumID and TransitionGroupID for timesteps >500
#mean is used here b/c iterations with 0 values are written to the sim database 
ClassResults2<-ClassResults1 %>%
...
#export results and raw data for last 500 timesteps to csv
write.csv(ClassResults2, file=paste(mylib, "ClassResults2.csv"))
write.csv(ClassResults1, file=paste(mylib, "ClassResults1.csv"))
